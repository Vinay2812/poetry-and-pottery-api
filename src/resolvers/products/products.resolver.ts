...
export class ProductsResolver {
  ...
  async productBySlug(
    ...
    return tryCatchAsync(async () => {
      ...
      const cachedProduct = await productCache.bySlug(slug, async () => {
  ...
  async bestSellers(
    ...
    return tryCatchAsync(async () => {
      ...
      const cachedData = await productCache.bestSellers(
        page,
        limit,
        async () => {
          const offset = (page - 1) * limit;
          const where = { is_active: true, available_quantity: { gt: 0 } };
          ...
          return {
            products: products.map((product) => mapToProductBase(product)),
            total,
            page,
  ...
  async categoriesWithImages(
    ...
    return tryCatchAsync(async () => {
      return productCache.categoriesWithImages(async () => {
        const categoryCounts = await ctx.prisma.productCategory.groupBy({
          by: ["category"],
          _count: { product_id: true },
          orderBy: { _count: { product_id: "desc" } },
        ...
        const categories = await Promise.all(
          categoryCounts.map(async (c) => {
            const productCategory = await ctx.prisma.productCategory.findFirst({
              ...
              include: { product: { select: { image_urls: true } } },
            });
            return {
              name: c.category,
  ...
  async materials(@Ctx() ctx: Context): Promise<string[]> {
    return tryCatchAsync(async () => {
      return productCache.materials(async () => {
